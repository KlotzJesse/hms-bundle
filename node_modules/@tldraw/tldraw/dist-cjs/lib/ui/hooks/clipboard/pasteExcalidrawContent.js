...
async function pasteExcalidrawContent(editor, clipboard, point) {
  ...
  const bounds = import_editor.Box2d.Common(rootShapes.map((s) => editor.getShapePageBounds(s)));
...
function getBend(element, startPoint, endPoint) {
  ...
  if (element.points.length > 2) {
    const start = new import_editor.Vec2d(startPoint[0], startPoint[1]);
    const end = new import_editor.Vec2d(endPoint[0], endPoint[1]);
    const handle = new import_editor.Vec2d(element.points[1][0], element.points[1][1]);
    ...
    const point = import_editor.Vec2d.NearestPointOnLineSegment(A, B, handle, false);
    bend = import_editor.Vec2d.Dist(point, med);
    if (import_editor.Vec2d.Clockwise(point, end, med))
      bend *= -1;
  }
  return bend;
}
