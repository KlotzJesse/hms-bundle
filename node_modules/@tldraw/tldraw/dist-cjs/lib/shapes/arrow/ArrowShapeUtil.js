...
class ArrowShapeUtil extends import_editor.ShapeUtil {
  ...
  onHandleChange = (shape, { handle, isPrecise }) => {
    ...
    if (precise) {
      if (import_editor.Vec2d.Dist(pointInTargetSpace, targetBounds.center) < Math.max(4, Math.min(Math.min(targetBounds.width, targetBounds.height) * 0.15, 16)) / this.editor.getZoomLevel()) {
  ...
  toSvg(shape, ctx) {
    const theme = (0, import_editor.getDefaultColorTheme)({ isDarkMode: this.editor.user.getIsDarkMode() });
