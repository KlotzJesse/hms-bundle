  ...
  for (let i = 0; i < strokePoints.length; i++) {
    ...
    if (isPointSharpCorner || isNextPointSharpCorner) {
      if (nextDpr > -0.62 && totalLength - strokePoint.runningLength > strokePoint.radius) {
        ...
        const start = import_editor.Vec2d.Sub(strokePoint.input, offset2);
        for (let step = 1 / 13, t = 0; t < 1; t += step) {
          tl = import_editor.Vec2d.RotWith(start, strokePoint.input, FIXED_PI * t);
          leftPts.push(tl);
          tr = import_editor.Vec2d.RotWith(start, strokePoint.input, FIXED_PI + FIXED_PI * -t);
          rightPts.push(tr);
        }
      }
      pl = tl;
