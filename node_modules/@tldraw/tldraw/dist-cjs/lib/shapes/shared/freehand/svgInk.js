...
function partitionAtElbows(points) {
  ...
  for (let i = 1; i < points.length - 1; i++) {
    ...
    const prevAngle = import_editor.Vec2d.Angle(prevPoint.point, thisPoint.point);
    const nextAngle = import_editor.Vec2d.Angle(thisPoint.point, nextPoint.point);
    ...
    const incomingNormalizedDist = import_editor.Vec2d.Dist(prevPoint.point, thisPoint.point) / avgRadius;
    const outgoingNormalizedDist = import_editor.Vec2d.Dist(thisPoint.point, nextPoint.point) / avgRadius;
...
function cleanUpPartition(partition) {
  ...
  while (partition.length > 2) {
    ...
    const dist = import_editor.Vec2d.Dist(startPoint.point, nextPoint.point);
  ...
  while (partition.length > 2) {
    ...
    const dist = import_editor.Vec2d.Dist(endPoint.point, prevPoint.point);
  ...
  if (partition.length > 1) {
    partition[0] = {
      ...partition[0],
      vector: import_editor.Vec2d.FromAngle(import_editor.Vec2d.Angle(partition[1].point, partition[0].point))
    };
    partition[partition.length - 1] = {
      ...partition[partition.length - 1],
      vector: import_editor.Vec2d.FromAngle(
        import_editor.Vec2d.Angle(partition[partition.length - 1].point, partition[partition.length - 2].point)
      )
    };
  }
  return partition;
...
function renderPartition(strokePoints, options = {}) {
  ...
  }
  {
    ...
    const arcStart = import_editor.Vec2d.Add(point.point, import_editor.Vec2d.Mul(direction, radius));
    const arcEnd = import_editor.Vec2d.Add(point.point, import_editor.Vec2d.Mul(direction, -radius));
